...
Scout 数据源只在 Extract 阶段访问。运行 `python scout/run_scout.py --fetch-only`
会把联网结果保存为 `scout/scout_runs/YYYYMMDD_HHMM_raw.json`；后续
`--from-raw` 评分只读取这份 raw snapshot，不再访问任何数据源。Cowork
评分必须先拿到 raw snapshot，不能直接执行联网抓取。
