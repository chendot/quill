...
│   └── scout_candidates.md  # Scout 生成的候选话题，每次运行覆盖
├── outputs/                 # 自动生成，不进 git
│   └── YYYYMMDD_HHMM/
│       ├── 01_research.md
│       ├── 02_strategy.md
...
│   ├── sources/
│   │   ├── __init__.py
│   │   ├── defillama.py     # DefiLlama TVL 变化
│   │   ├── eastmoney.py     # 东方财富板块资金流
│   │   ├── http.py          # 轻量 JSON 请求 helper
│   │   └── polymarket.py    # Polymarket 概率变化
...
- Scout 与 `pipeline/` 完全解耦；主 pipeline 不 import Scout。
- Scout 输出只写入 `inputs/scout_candidates.md`，供人阅读和筛选。
...
| Source | API | 抓取规则 | 输出字段 | 失败处理 |
|--------|-----|----------|----------|----------|
| DefiLlama | `https://api.llama.fi/protocols` | 7日 TVL 变化幅度 > 20% 的协议 | 协议名、当前 TVL、7日变化%、所属链、类别 | 跳过并标注不可用 |
| Eastmoney | `https://push2.eastmoney.com/api/qt/ulist.np/get`，必要时同域 `clist/get` fallback | 当日主力资金净流入 top5 板块 + 净流出 top5 板块 | 板块名、涨跌幅、主力净流入金额 | 跳过并标注不可用 |
| Polymarket | `https://gamma-api.polymarket.com/markets?limit=20&order=volume&ascending=false` | 成交量 top20 中概率在 `[0.15, 0.85]` 的市场 | 市场名称、当前概率、24h 概率变化、成交量 | 跳过并标注不可用 |
...
数据源：defillama, eastmoney, polymarket
---

## 01 · [话题标题，一句话]
评分：8/10 | 来源：DefiLlama | 证据等级：A
数据摘要：[2-3句，只写事实，不写观点]
反直觉角度：[为什么值得写，一句话]
建议切入点：[适合的内容角度，一句话]
```
...
1. 运行 `python scout/run_scout.py`
2. 打开 `inputs/scout_candidates.md` 查看候选
3. 挑选一条，手动编辑 `inputs/idea.md`
4. 运行 `python run.py --provider groq` 或其他主 pipeline 命令
...
1. 创建 `scout/` 目录结构和 `scout/.gitignore`
2. 先实现 DefiLlama 数据抓取和 `writer.py`
3. 跑通 `python scout/run_scout.py --sources defillama`
4. 增加 Eastmoney 和 Polymarket 数据源
5. 最后增加 `scout/scorer.py` 的 LLM 评分
6. 完整联调 `python scout/run_scout.py --provider groq --top 5`
