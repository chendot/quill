...
Transform rules:
- Transform scoring records two separate components: `popularity_score`
  (heat/timeliness/source/tier signal) and `argumentability_score`
  (可论证性). LLM scoring writes both values; rule-only scoring writes
  `argumentability_score: null`.
- Track format is `领域×子方向`. LLM scoring assigns track in the same scoring
  call, with a target of roughly 2-5 items per track in one batch. Rule-only
  scoring degrades to keyword-based track inference.
...
idea.md output rules:
- Existing fields (title, source, track, angle, platform) keep their current
  format and position
- A `## Scout 预判` block is appended at the end, generated by one LLM call
  using the candidate's raw snapshot materials as the only input
- The block contains exactly four fields: 核心张力, 支持方最强论据,
  反对方最强论据, 最关键的不确定性
- Each field must be derivable from raw snapshot fields only; if a field cannot
  be inferred from available material, write "材料不足，无法判断" — never invent
- The LLM prompt for this block lives in `scout/prompts/prepare_judgment.md`

data.md output rules:
- Items are organized into three tiers instead of a flat list:
  - 一级素材: facts or data that directly support the core thesis, with source
  - 二级素材: background or indirect supporting material
  - 待核实: claims present in the raw snapshot that lack source or data support
- Each item retains: source label, evidence_grade from the raw snapshot item,
  and a one-sentence annotation of which argument direction it supports
- If a raw snapshot item has no evidence_grade field, omit the grade annotation
  without erroring

同源上下文 filter rules:
- Include only raw-snapshot items whose `track` exactly matches the selected
  candidate's `track`
- Do not fall back to source or category matching as substitutes for track match
- If no other item in the same raw snapshot shares the candidate's `track`,
  omit the 同源上下文 section entirely — never backfill with unrelated items
