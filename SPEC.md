...
设计原则：
- 线性 forge，不用 Agent 框架
- 每步输出单独存文件，支持断点重跑
- Prompt 完全可控，存于独立 md 文件
- 内容哲学优先于自动化效率
- Researcher 不捏造事实，只标注数据缺口
- 合规检查双轨：规则函数兜底 + LLM 语气风险判断
- 支持三种运行模式：**API 模式**（调用外部 LLM provider）、**Cowork 模式**（Claude 原生执行，无需额外 API 调用）和 **Codex 模式**（Codex 原生执行，无需额外 API 调用）
- 支持通过 `--platform` 指定正文输出平台，默认 `x-thread`
- 可选独立 Scout 模块用于发现候选话题，但不进入主 forge、不自动改写 idea.md
...
- 无 LangChain / CrewAI / LangGraph，纯函数式 forge
- 无数据库，文件系统即状态
...
**Cowork/Codex 模式**：脚本不阻塞，HITL 在对话中进行。步骤 02 完成后对话侧 assistant 向用户展示选题报告和标题候选，用户在对话中确认后 assistant 继续运行下一步；步骤 06 完成后 assistant 展示最终稿，由用户决定是否发布。
...
**Codex 模式**（Codex 原生执行，在 Codex 对话中调用）：
```
python run.py --provider codex             # 准备步骤 01，打印 prompt+input，退出
python run.py --provider codex --from 02 --dir 20260626_1520  # 续跑步骤 02
python run.py --provider codex --from 03 --dir 20260626_1520 --platform xhs-caption # 用小红书配图说明格式准备步骤 03
python run.py --provider codex --from done --dir 20260626_1520 # 所有步骤完成后运行合规扫描
```
Codex 模式每次调用只处理一个步骤：脚本打印当前步骤的 system prompt 和 user input，写入 `.codex_step.json`，由 Codex 生成输出并写入文件，再调用下一步命令。HITL 在对话中处理，脚本不阻塞。
...
Scout 是一个可选的、独立运行的话题发现工具，用于把公开数据源中的异常变化整理成候选选题。它不是主 forge 的一个步骤，不改变 `run.py` 的线性执行顺序，也不会自动写入 `inputs/idea.md`。

### 设计边界

- 主 forge 不 import Scout，Scout 不改变主 forge 的执行顺序或状态；Scout 可复用 `forge/loader.py`、`forge/runner.py` 中的通用 prompt/LLM helper。
- Scout 输出只写入 `inputs/scout_candidates.md`，供人阅读和筛选；该文件是运行产物，不进 git。
- 用户必须手动把选中的候选改写进 `inputs/idea.md`，再运行主 forge。
...
Scout provider 优先级：`--provider` 命令行参数 > `.env` 中的 `DEFAULT_PROVIDER` > 默认 `groq`。支持 `groq` / `gemini` / `anthropic` 作为 API 评分 provider，也支持 `cowork` / `codex` 由当前对话中的 assistant 直接评分。
...
Cowork/Codex 模式禁止执行联网抓取。若未传 `--from-raw`，脚本必须报错并提示先在本机运行 `--fetch-only` 生成 raw snapshot。拿到 raw snapshot 后，脚本会：

1. 写入 `scout/scout_runs/YYYYMMDD_HHMM_cowork.json` 或 `scout/scout_runs/YYYYMMDD_HHMM_codex.json`
2. 打印 Scout scorer 的 system prompt 和 user input
3. 指明需要覆盖写入的 `inputs/scout_candidates.md`
4. 指明需要同步写入的归档文件路径

当前对话中的 assistant 根据打印的 prompt 直接完成评分和文件写入。Cowork/Codex 模式不阻塞 Python，也不自动进入主 forge。
...
5. 运行 `python run.py --provider groq` 或其他主 forge 命令
...
PROVIDER_MODELS = {
    ...
    "codex": PRIMARY_MODEL,            # Codex 模式：Codex 直接执行，无外部 API 调用
}
...
└── 20260626_1430/
    ...
    └── .codex_step.json    # Codex 模式临时文件：当前步骤的 prompt+input 清单
```
...
`--from done` 是 Cowork/Codex 模式专用的特殊值，表示所有 LLM 步骤已完成，仅运行最终合规扫描（`scan_hard_rules`）并写入 meta.json。
...
3. 实现 `forge/compliance.py`（纯规则，无 LLM 依赖，最先可测试）
4. 实现 `forge/runner.py`（含 token 统计和重试逻辑）
5. 实现 `forge/loader.py` 和 `forge/writer.py`
...
- 多并发 Agent
- 自动生成图表（出图逻辑独立维护）
- 主 forge 的 web search 接入（Phase 2，Researcher 真正补数据时再加；Scout 的固定公开 API 不等同于 Researcher web search）
