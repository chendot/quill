...
第一批默认可用源：arXiv、GitHub Trending、Hacker News、DefiLlama、Polymarket。

第二批可选源：Hugging Face Papers、FRED、Google Trends。FRED 需要 `FRED_API_KEY`；Google Trends 需要安装 `pytrends`，被限速或未安装时跳过并在输出中标注不可用。
...
|--------|-----|----------|----------|
| arXiv | `http://export.arxiv.org/api/query` | `cs.AI` + `q-fin` 最近 48 小时新论文，按提交时间取最新 20 篇 | 标题、作者、摘要前 200 字、提交时间、链接 |
| GitHub Trending | `https://github.com/trending?since=daily` | 今日 trending，语言不限，取前 20 | repo 名、描述、star 数、今日新增 star、语言、链接 |
...
| DefiLlama | `https://api.llama.fi/protocols` | 7日 TVL 变化幅度 > 20% 的协议 | 协议名、当前 TVL、7日变化%、所属链、类别 |
| Polymarket | `https://gamma-api.polymarket.com/markets?limit=20&order=volume&ascending=false` | 成交量 top20 中概率在 `[0.15, 0.85]` 的市场 | 市场名称、当前概率、24h 概率变化、成交量 |
| FRED | `https://api.stlouisfed.org/fred/series/observations` | FEDFUNDS、DGS10、DTWEXBGS、CPIAUCSL 最新值和约 30 日变化 | 指标名、最新值、日期、30 日变化 |
